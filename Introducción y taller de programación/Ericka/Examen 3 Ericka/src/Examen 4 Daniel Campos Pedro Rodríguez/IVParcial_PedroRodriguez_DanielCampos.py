...
def papeletasEnEstado(UrnaId,Situacion):
    ...
    if Situacion=="V":
        ...
        return "Error, condicion no valida"
...
def ganador():
    ...
    if votosContados.count(votosganador)>1:
        return "Hubo un empate entre candidatos"
    else:
...
#---------------------------------------------------------------
simula=input("Bienvienido. Digitar 1 por si desea una simulación automatica o cualquier otro numero para simulacion manual... (Si inserta algo distinto a un numero se invocará la función automática ")
if simula==1 or not isinstance(simula,int):
    cantidadpadron=random.randint(50,200)
else:
    seguir=False
    while not seguir:
        try:
            cantidadpadron=input("Favor insertar el numero (mayor a cero) de estudiantes total para crear el padron estudiantil: ")
            if isinstance(cantidadpadron,int):
                if cantidadpadron>0:
                    seguir=True
                else:
                    print("Lo insertado no es un numero mayor a cero. Favor intentar de nuevo.")
            else:
                print("Lo insertado no es un numero. Favor intentar de nuevo.")
        except:
            print("Lo insertado no es un numero. Favor intentar de nuevo.")
i=1
print ("====================================================================")
...
print ("====================================================================")
crearUrnas()
print ("Durante las votaciones, se utilizaron " + str(len(BallotBoxDataBase)) + " urnas.")
UrnasIds=[]
for i in BallotBoxDataBase:
    urna=i.getUrna()
    urna=urna[0]
    if urna not in UrnasIds:
        UrnasIds.append(urna)
print ("Lista de ID de urnas: \n"+str(UrnasIds)+"\n")
if simula==1:
    for i in BallotBoxDataBase:
        urnaId=i.getUrna()
        urnaId=urnaId[0]
    ...
    seguir=False
    while not seguir:
        cond=input("Estamos a pocas horas de cerrar las votaciones\n1. Ver situación de urnas\n2. Seguir votaciones\n")
        if cond==1:
            id=input("Favor insertar Id de la urna: ")
            situacionUrna(id)
            seguir=False
        elif cond==2:
            seguir=True
        else:
            print ("Error, favor intentar de nuevo")
votaciones()
print ("................................................!")
print ("====================================================================")
...
if simula==1:
    for i in UrnasIds:
        ...
        print papeletasEnEstado(i,"V")###papeletasEnEstado(id,estado)
    ...
    i=1
    while not seguir:
        cond=input("1. Ver estado de papeletas en una urna específica\n2. Ver resultado y el ganador (Aquí se saldrá del programa)\n")
        if cond==1:
            iden=input("Favor insertar Id de la urna: ")
            if iden not in UrnasIds:
                print ("Error, tal urna no existe\n")
            else:
                try:
                    situa=input("Favor insertar (entre comillas) V para votos válidos, B para votos en blancos y N para votos nulos: ")
                    print papeletasEnEstado(iden,situa)+"\n"
                except:
                    print ("Error, datos no fueron insertados correctamente\n")
        elif cond==2:
            print ("====================================================================")
            print ("                          Resultado                                 ")
            print ("====================================================================")
